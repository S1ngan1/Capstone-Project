...
export const testStorageSetup = async () => {
  try {
    ...
    if (bucketsError) {
      ...
      Alert.alert('Storage Test Failed', `Cannot list buckets: ${bucketsError.message}`);
      return;
    }
    ...
    if (filesError) {
      ...
      Alert.alert('Storage Test Failed', `Cannot list files: ${filesError.message}`);
      return;
    }
    ...
    if (uploadError) {
      ...
      Alert.alert('Upload Test Failed', `Cannot upload test file: ${uploadError.message}`);
      return;
    }
    ...
    Alert.alert('Storage Test Passed', 'Your storage configuration is working correctly!');
    
  } catch (error) {
    console.error('Storage test error:', error);
    Alert.alert('Storage Test Error', `Unexpected error: ${error}`);
  }
};